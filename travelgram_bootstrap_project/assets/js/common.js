document.addEventListener("DOMContentLoaded", () => {
  ...
  // 모든 페이지가 완전히 같은 사이드바 HTML을 사용하고,
  // 현재 파일명에 따라서만 active 상태를 자동으로 표시합니다.
  const currentFile = window.location.pathname.split("/").pop() || "index.html";
  const pageToNavKey = {
    "index.html": "home",
    "feed-text.html": "home",
    "post-detail.html": "home",
    "travelgram.html": "travelgram",
    "chat.html": "chat",
    "tag.html": "tag",
    "mission.html": "mission",
    "mission-active.html": "mission",
    "profile.html": "profile",
    "login.html": "profile",
    "signup.html": "profile",
    "forgot-password.html": "profile",
    "new-post.html": "new"
  };

  const activeNavKey = pageToNavKey[currentFile] || "home";

  document.querySelectorAll("[data-nav-key]").forEach((link) => {
    const isCurrent = link.dataset.navKey === activeNavKey;
  ...
  document.querySelectorAll("[data-mission-accept]").forEach((button) => {
    button.addEventListener("click", () => {
      ...
      window.location.href = `mission-active.html?mission=${mission}`;
    });
  });
