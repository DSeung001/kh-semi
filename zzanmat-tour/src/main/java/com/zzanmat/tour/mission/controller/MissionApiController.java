...
public class MissionApiController {
    ...
    // 4. 체크리스트 상태 조회 API (String으로 받아 '1:1' 및 변환 오류 원천 차단)
    @GetMapping("/progress")
    public ApiResponse<Map<String, Boolean>> getMissionProgressStatus(
        ...
        return ApiResponse.success(missionService.getUserChecklistStatus(userId, parsedMissionId));
    }
    ...
    public ApiResponse<Void> acceptMission(
        ...
        if (userId == null) {
            log.warn("미션 수락 실패: 로그인되지 않은 사용자 (세션에 유저 정보 없음)");
            throw new IllegalArgumentException("로그인이 필요합니다.");
        }
        ...
        try {
            ...
            log.error("미션 수락 중 서비스 에러 발생: {}", e.getMessage(), e);
            throw e;
        }
    ...
    public ApiResponse<MissionResponseDto.UserMissionDetail> completeMission(
        ...
        Long safeMissionId = requestDto.getMissionId();
        MissionResponseDto.UserMissionDetail response = missionService.completeMission(userId, safeMissionId);
    ...
    /**
     * 9. 로그인 세션 저장 방식을 모두 수용하는 안전한 유저 ID 추출 메서드
     */
    private Long getUserId(HttpSession session) {
        ...
        log.info("=== [디버깅] 현재 세션에 저장된 속성 목록 ===");
        while (attributeNames.hasMoreElements()) {
            String name = attributeNames.nextElement();
            log.info("Session Key: [{}], Value: [{}]", name, session.getAttribute(name));
        }
