...
public class MissionController {
    ...
    // 미션 수락 전 로그인 및 인증 검증 (POST)
    @PostMapping("/check-auth-and-accept")
    public String checkAuthAndAccept(
            @RequestParam("missionId") String missionId,
            @RequestParam(value = "redirectUrl", required = false) String redirectUrl,
            HttpSession session) {

        Object loginMember = session.getAttribute(SessionConst.LOGIN_MEMBER);
        if (loginMember == null) loginMember = session.getAttribute("loginUser");
    ...
    // 인증 게시물 작성 전 로그인 검증 (POST)
    @PostMapping("/check-auth-and-post")
    public String checkAuthAndPost(
        ...
        public String checkMissionAuth(
